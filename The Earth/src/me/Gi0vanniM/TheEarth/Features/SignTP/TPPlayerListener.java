	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			if ((block.getState() instanceof Sign)) {
				...
				if ((CoordinateSign.isSignWarp(sign)) && (player.hasPermission("SignTP.Use.Click"))) {
					String warpName = sign.getLine(0).substring(1, sign.getLine(0).length() - 1);
					event.setCancelled(true);
					CoordinateSign.warpSign((Sign) block.getState(), event.getPlayer());
		...
		if (event.getAction().equals(Action.PHYSICAL)) {
			if (event.getClickedBlock().getType() == Material.IRON_PLATE) {
				...
				if ((block.getState() instanceof Sign)) {
					...
					if ((CoordinateSign.isSignWarp(sign)) && (player.hasPermission("SignTP.Use"))) {
...
//						player.sendMessage(ChatColor.DARK_AQUA + "You have been teleported.");
	...
	@EventHandler
	public void elytralaunch(PlayerInteractEvent event) {
		if (event.getAction().equals(Action.PHYSICAL)) {
			if (event.getClickedBlock().getType() == Material.STONE_PLATE) {
				Player player = event.getPlayer();
				Block block = event.getClickedBlock().getRelative(BlockFace.DOWN);
				if (event.getClickedBlock().getLocation().add(0.0D, -1.0D, 0.0D).getBlock().getType()
						.equals(Material.BEDROCK)) {

					if ((player.hasPermission("elytralaunch.Use"))) {
						...
						if (this.cooldowns.containsKey(player.getName())) {
							long secondsLeft = ((Long) this.cooldowns.get(player.getName())).longValue() / 1000L
									+ cooldownTime - System.currentTimeMillis() / 1000L;
							if (secondsLeft > 0L) {
								player.sendMessage(ChatColor.GRAY + "You can launch again in: §c" + ChatColor.RED
										+ secondsLeft + ChatColor.GRAY + " second(s).");
								// Main.sendActionBar(player, Main.plugin.title + ChatColor.GRAY + "Wacht §c" +
								// ChatColor.RED + secondsLeft + ChatColor.GRAY + " seconden.");
								// player.getWorld().playSound(player.getLocation(), Sound.BLAZE_BREATH, 1.0F,
						...
						player.sendMessage(ChatColor.DARK_AQUA + "You have been launched.");

						player.getWorld().playSound(player.getLocation(), Sound.ENTITY_ENDERMEN_TELEPORT, 10.0F, 1.0F);
					}
				}
			}
		}
